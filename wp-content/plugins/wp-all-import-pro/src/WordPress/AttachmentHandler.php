...
class AttachmentHandler{
	...
	public function __construct($articleData, $pid, $missing_images = [], $postAuthor = [], $postType = [], $i = 0) {
		...
		if (!empty(self::$importData['current_xml_node'])) {
			$current_xml_node = self::$importData['current_xml_node'];
		}
		...
		self::$user_agent = $mimicBrowserUserAgent ? 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36' : self::$user_agent;

		self::$user_agent = apply_filters('pmxi_user_agent', self::$user_agent);
